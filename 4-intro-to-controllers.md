...
* What the hell is `$scope`?
	-$scope is an object that refers to the application model. It is the execution context for an expression and is arranged in a hierarchical struture as the DOM is. 
* What are Angular modules?
	-A module is a container for different parts of an app that can be for controllers, directives, filters. It is where the above controller was declared. 
* Why do we pass in `$scope` as an argument to controller functions?
	-Its much clearer what the paramenters the function expects and you can pass in the logic of a function into an expression. Also, it's much easier to unit test because all parameters are injected into the function. 
